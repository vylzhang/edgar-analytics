...
def run():
	...
	# save the column position of the notable fields
	for i in xrange(len(header)):
		if header[i] in fields:
			cols[header[i]] = i
	
	# read rest of input
	## {users} = ip: [start_request_time, count, last_request_time] <- upon encountering ip, update count and last_request_time
	## [expiry_time] = [ip] <- at expiry time, remove matches (from this list and users dict) and write to output file
	users = {}
	...
	for i, line in enumerate(f):
		...
		if current_time != prev_time: # we are now looking at a new time, time to check expiry list
